...
- (instancetype)initCardAlertWithTitle:(NSString*)title
    ...
    self.imgUrl = [NSURL URLWithString:string];
    [self configureViewForAlertType:type iconImage:iconImage backgroundImageString:string];
...
- (void)constructAlertCardView {
    ...
    [_bgImage setFrame:alertView.frame];
    [alertView addSubview:_bgImage];
    [alertView sendSubviewToBack:_bgImage];
...
- (void)show:(handler)handler {
    ...
    NSURLRequest *req = [NSURLRequest requestWithURL:self.imgUrl];
    ...
    [_bgImage setImageWithURLRequest:req placeholderImage:nil success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
...
- (void)configureViewForAlertType:(ISAlertType)alertType iconImage:(UIImage*)iconImage backgroundImageString:(NSString*)string {
    ...
    switch (alertType) {
        ...
        case ISAlertTypeFriends: {
            self.alertViewBackgroundColor = [self mergedColorFromImage:_bgImage.image];
            if (!_iconImage) {
                self.iconImage = [self imageNamed:@"isSuccessIcon"];
            }
            break;
...
- (UIColor *)mergedColorFromImage:(UIImage*)image
{
    CGSize size = {1, 1};
    ...
    UIColor *color = [UIColor colorWithRed:data[2] / 400.0f
                                     green:data[1] / 400.0f
                                      blue:data[0] / 400.0f
                                     alpha:1];
